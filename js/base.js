      ...
      $(window).scroll(function () {
        ...
        if ($(window).scrollTop() > $('.right').offset().top + right_height) {
          $('.self_info_scroll_bottom')
            .stop()
            .animate({ top: 0, right: right_right, opacity: 1 })
        } else if (
          $(window).scrollTop() <=
          $('.right').offset().top + right_height
        ) {
          $('.self_info_scroll_bottom')
            .stop()
...
function click_show_hide_article_btn_box_item() {
  $('.article_btn ul li').click(function () {
    ...
    $(this)
      .css({ 'border-bottom': '1px solid #004FCB', color: '#004FCB' })
      .siblings()
      .css({ 'border-bottom': 'none', color: '#333' })
    ...
    if (_thisitem.find('ul li:first').find('a').length > 0) {
      _thisitem
        .find('.article_btn_box_item_img')
        .find('img')
        .attr('src', _thisitem.find('ul li:first').find('a').attr('imgsrc'))
      _thisitem
        .find('.article_btn_box_item_img')
        .attr(
          'href',
...
function mousehover_changeimgsrc() {
  ...
  _first_box
    .find('.article_btn_box_item_img')
    .find('img')
    .attr('src', _first_box.find('ul li:first').find('a').attr('imgsrc'))
  _first_box
    .find('.article_btn_box_item_img')
    .attr('href', _first_box.find('ul li:first').find('a').attr('href'))
  var _itemfather = $('.article_btn_box .article_btn_box_item')
  _itemfather.find('ul li').hover(
    function () {
      ...
      $(this)
        ...
        .parent()
        .find('img')
        .attr('src', $(this).find('a').attr('imgsrc'))
      $(this)
        .parent()
        .parent()
        .find('.article_btn_box_item_img')
    ...
    function () {
      $(this)
        ...
        .parent()
        .find('img')
        .attr('src', $(this).parent().find('li:first').find('a').attr('imgsrc'))
      $(this)
        .parent()
        .parent()
        .find('.article_btn_box_item_img')
        .attr('href', $(this).parent().find('li:first').find('a').attr('href'))
    }
  )
}
//手机端页面按钮点击三横变一×
